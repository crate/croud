...
from tests.unit_tests.util.fake_server import FakeCrateDBCloud, FakeResolver
...
class TestHttpSession:
    ...
    @pytest.mark.parametrize("params", [None, {"a": "foo", "b": 42}])
    @mock.patch.object(Configuration, "get_token", return_value="eyJraWQiOiIx")
    def test_rest_success(self, mock_token, mock_env, params):
        with loop_context() as loop:
            # Takes a query that will be used to determine which response is
            # generated by the FakeCrateDBCloud instance
            async def test_request():
                ...
                async with HttpSession(
                    ...
                    result = await session.rest_fetch(
                        RequestMethod.POST, params, "/rest_me"
                    )
                    assert result["data"] == me_response
    ...
    @mock.patch.object(Configuration, "get_token", return_value="")
    @mock.patch("croud.session.print_error")
    def test_rest_unauthorized(self, mock_print_error, mock_token, mock_env):
        with loop_context() as loop:
            # Takes a query that will be used to determine which response is
            # generated by the FakeCrateDBCloud instance
            async def test_request():
                ...
                with pytest.raises(SystemExit) as cm:
                    async with HttpSession(
                        ...
                        await session.rest_fetch(RequestMethod.POST, {}, "/rest_me")
                mock_print_error.assert_called_once_with(
                    "Unauthorized. Use `croud login` to login to CrateDB Cloud."
                )
                assert cm.value.code == 1
    ...
    @mock.patch.object(Configuration, "get_token", return_value="eyJraWQiOiIx")
    @mock.patch("croud.session.print_info")
    def test_rest_failed(self, mock_print_info, mock_token, mock_env):
        with loop_context() as loop:
            # Takes a query that will be used to determine which response is
            # generated by the FakeCrateDBCloud instance
            async def test_request():
                ...
                async with HttpSession(
                    ...
                    await session.rest_fetch(RequestMethod.GET, {}, "/rest_fail")
                    mock_print_info.assert_has_calls(
                        [
                            mock.call(
                                "Query failed to run by returning code of 400. "
                                "Method: GET Endpoint: /api/v2/rest_fail"
                            )
                        ]
                    )
